  ...
  export default {
    ...
    methods: {
      ...
      async postActivity () {
        ...
        var attachmentRef = storageRef.child(`posts/${newPostKey}/attachment.jpg`)
        try {
        ...
        try {
          if (this.activityPhotoFileName !== '') {
            attachmentUrl = await storageRef.child(`posts/${newPostKey}/attachment.jpg`).getDownloadURL()
          }
        } catch (error) {
          let snackbar = {
            active: true,
        ...
        try {
          await this.$firebase.database().ref('posts/' + newPostKey).setWithPriority(post, 1 * Date.now())
          this.content = ''
          this.cancelActivityPhoto()
          let snackbar = {
            active: true,
            text: 'Post was created successfully, but needs to be approved by the admin'
          }
          this.$store.dispatch('triggerSnackbar', snackbar)
        } catch (error) {
          let snackbar = {
