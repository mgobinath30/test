<template>
    <v-card v-resize="onResize" style="position: fixed;" :style="{ 'width': `${width}px`  }">
      <v-layout class="info-bg light-blue darken-2" justify-center>
      </v-layout>
      <v-layout class="info-profile-img" justify-center>
        <!--<img :alt="user && user.avatar ? user.avatar : 'default-avatar.gif'" :src="user && user.avatar ? user.avatar : 'default-avatar.gif'" 
        style="max-width:100%;max-height:150px;"> -->
        <img :alt="user && user.avatar ? user.avatar : 'default-avatar.gif'" :src="usercompanyLogo" 
        style="max-width:100%;max-height:150px;"> 
      </v-layout>
