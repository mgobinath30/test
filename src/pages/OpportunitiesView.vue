              ...
              >
                <template slot="items" scope="props">
                  <tr style="cursor: pointer" @click.prevent="moreInfo(props.item)">
                    ...
                    <td class="text-xs-left textLeft">{{ props.item.primarypointofcontact }}</td>
                  </tr>
                </template>
              </v-data-table>
            </v-flex>
  ...
  export default {
    ...
    watch: {
      pagination (val) {
        ...
        this.$store.dispatch('getOpportunities', { type: 'active', skip, limit: val.rowsPerPage }).then(res => {
    ...
    methods: {
      ...
      saveFilters () {
        this.$firebase.database().ref(`filters/activeopportunities/${this.$store.getters.getUserDetails.uid}`).set(this.filters)
      },
      moreInfo (item) {
        this.dialog = true
